...
public class DynamicTableService {
    ...
    public void createOrUpdateTable(String slug, List<ColumnDefinition> columnDefinitions) {
        ...
        if (!tableExists(slug)) {
            createTable(slug);
            columnDefinitions.forEach(c -> addColumn(slug, c.getColumnName(), c.getColumnType()));
        }
    }
    ...
    public Map<String, Object> getFilteredData(String slug, int page, int rowsPerPage, String search, String filter, String sortBy, String sortOrder) {
        ...
        String countSql = "SELECT COUNT(*) FROM " + slug
                + getQueryBuilder(search, " WHERE ")
                + getQueryBuilder(filter, search != null ? " AND " : " WHERE ");
    ...
    public boolean parseFloat(String s) {
    ...
    private List<Map<String, Object>> getParsedData(List<Map<String, Object>> rawData) {
        ...
        for (Map<String, Object> row : rawData) {
            ...
            if (dataObject != null) {
                String jsonDataString = dataObject.getValue();
                try {
                    ...
                    if (jsonData.isArray()) {
                        ...
                        for (JsonNode node : jsonData) {
                            parsedValue.add(objectMapper.convertValue(node, new TypeReference<Map<String, Object>>() {}));
                        }
    ...
    private StringBuilder getSortClause(String sortBy, String sortOrder) {
        ...
        if ("created_at".equals(sortBy) || "updated_at".equals(sortBy)) {
            ...
            for (int i = 0; i < keys.length; ++i) {
                if (i > 0) {
                    sortClause.append(", ").append("'").append(keys[i]).append("'");
                } else {
                    sortClause.append("\"").append(keys[i]).append("\"");
                }
            }
    ...
    public void deleteDataById(String slug, Long id) {
        ...
        if (deletedRows <= 0) {
            throw new IllegalStateException("Failed to delete row by ID from table: " + slug);
        }
    }
    ...
    private void createTable(String tableName) {
        try {
            jdbcTemplate.execute("CREATE TABLE " + tableName + " (id SERIAL PRIMARY KEY)");
        } catch (DataAccessException e) {
    ...
    public boolean isEntityPublicPost(String slug) {
        ...
        if (!tableExists(slug)) return false;
        Map<String, Object> mainDetails = getMainDetails(slug);
        return mainDetails != null && Boolean.TRUE.equals(mainDetails.get("public_entity_post"));
    }
    ...
    public boolean isEntityPublicGet(String slug) {
        ...
        if (!tableExists(slug)) return false;
        Map<String, Object> mainDetails = getMainDetails(slug);
        return mainDetails != null && Boolean.TRUE.equals(mainDetails.get("public_entity_get"));
    }
    ...
    private void extractElementsFromElement(JsonNode element, List<String> labels, List<Map<String, Object>> extractedElements) {
        Map<String, List<Map<String, Object>>> extractedElement = new HashMap<>();
        for (String label : labels) {
            JsonNode childNode = element.path(label);
            if (childNode.isMissingNode()) continue;
            extractedElement.put(label, convertNodeToMap(childNode));
        }
        if (!extractedElement.isEmpty() && !extractedElements.contains(extractedElement)) {
            extractedElements.add((Map) extractedElement);
        }
    }
    ...
    private List<Map<String, Object>> convertNodeToMap(JsonNode node) {
        ...
        if (node.isArray()) {
            for (JsonNode element : node) {
                resultList.add(convertElementToMap(element));
            }
        } else {
            resultList.add(convertElementToMap(node));
        }
        return resultList;
    }
    ...
    private Map<String, Object> convertElementToMap(JsonNode element) {
        ...
        if (element.isObject()) {
            elementMap = objectMapper.convertValue(element, new TypeReference<Map<String, Object>>() {});
