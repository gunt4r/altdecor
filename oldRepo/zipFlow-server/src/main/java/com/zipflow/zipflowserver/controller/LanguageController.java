...
public class LanguageController {
    ...
    public ResponseEntity<LanguageEntity> getLanguageById(@PathVariable Long id) {
        ...
        return languageEntity.map(ResponseEntity::ok).orElseGet(() -> ResponseEntity.notFound().build());
    }
    ...
    @PutMapping("/{id}")
    public ResponseEntity<LanguageEntity> updateLanguage(@PathVariable Long id, @Valid @RequestBody LanguageEntity updatedLanguage) {
        if (!languageRepository.existsById(id)) {
            return ResponseEntity.notFound().build();
        }
        updatedLanguage.setId(id);
        LanguageEntity savedLanguage = languageRepository.save(updatedLanguage);
