...
export class ProductsComponent implements OnInit {
  ...
  setNestedFilters(nestedCategoryValue: any, params?: any) {
    if (nestedCategoryValue?.length) {
      ...
      const mCategory = this.categories?.data?.find((mainCategory: any) => findObjectByKey(mainCategory.data, 'label')?.[language] === nestedCategoryValue[0]?.[language]);
      ...
      if (!key.includes('product_type') &&
        ...
        if (event[key]) {
          filterParams = [...filterParams, ...event[key].map((data: any, index: number) => (
            {
              key: key,
              value: data?.[localStorage.getItem('language') || 'ro'],
              linkWord: LinkWord.CONTAINS,
